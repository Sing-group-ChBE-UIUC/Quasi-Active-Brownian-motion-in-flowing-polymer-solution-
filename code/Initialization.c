...
void ParseInput(int argc, char * argv[]){
    ...
    while((option = getopt(argc, argv, "c:b:a:t:e:d:p:n:r:f:s:z:i:m:g:")) != -1){
        switch(option){
...
void initBox(){
    ...
    box_length = L;//april_28
    box_side = L/2; // Box centered at origin, so sides at +- L/2
    //printf("%lf",box_side);

    outp = malloc(sizeof(char)*100);
    sprintf(outp, "txt/P%d_%d_%.3lf_%.5lf_%d_%d.txt", Nb, Nc, c_norm, flowrate, nt_avg, trace);
    // printf("%s\n",outp);
...
void initLattice(){
    ...
    xmax = point[2][0];
    zmax = L/2;
    // Return to undeformed box to find max height in rotated frame
...
void initVerlet(){
    ...
    rc = 2.5*sigma; // LJ cutoff range (assumption: goes to zero outside this range)//april_17
    r2cut = rc*rc;
    //r2cut = 5.03968;//commented off april_18
    //rc = sqrt(r2cut);//commented off april_18
    //rv =  3.0*sigma; // Verlet list radius. Particles inside this radius are added to the varlet list com_april_8
    rv =  5.0*sigma;//april_8
    rv2 = rv*rv;
    rnew = 0.5*(rv-rc);
    nlist = calloc(N+1, sizeof(long int)); // Number of neighbors in neighbor list for each bead april_10
    ...
    list = calloc((N+1)*(N+1), sizeof(long int));//april_10
    // Displacement from the bead position the last time neighbor tables were updated
    dxv = calloc(N+1,sizeof(double));
    dyv = calloc(N+1,sizeof(double));
    dzv = calloc(N+1,sizeof(double));
        }

void initCell(){ //april_28
    int i,j;
    sigma = 2.0;
    rc = 2.5*sigma;
    r2cut = rc*rc;
    ...
    ll = calloc(N+1,sizeof(int));//april_5
    // printf("%lf %d %lf\n",rc,rn,rl);
}
...
void allocate(){
    ...
    sprintf(xyz, "xyz/R%d_%d_%.3lf_%.5lf_%d_%d_%d.xyz", Nb, Nc, c_norm, flowrate, nt_avg, trace, iterstart);
    sprintf(matrix, "mm/M%d_%d_%.3lf_%.5lf_%d_%d.txt", Nb, Nc, c_norm, flowrate, trace, iterstart);
    sprintf(grid, "mm/G%d_%d_%.3lf_%d.txt", Nb, Nc, c_norm, nt_avg);
    sprintf(decomp, "decomp/DC%d_%d_%.3lf_%.5lf_%d_%d.txt", Nb, Nc, c_norm, flowrate, trace, iterstart);
    sprintf(beta, "decomp/B%d_%d_%.3lf_%.5lf_%d_%d.txt", Nb, Nc, c_norm, flowrate, trace, iterstart);
    sprintf(ext, "prop/E%d_%d_%.3lf_%.5lf_%d_%d_%d.txt", Nb, Nc, c_norm, flowrate, nt_avg, trace, iterstart);
    sprintf(visc, "prop/VF%d_%d_%.3lf_%.5lf_%d_%d_%d.txt", Nb, Nc, c_norm, flowrate, nt_avg, trace, iterstart);
    sprintf(tim, "prop/T%d_%d_%.3lf_%.5lf_%d_%d_%d.txt", Nb, Nc, c_norm, flowrate, nt_avg, trace, iterstart);
    rx = calloc(N+1, sizeof(double));
    ry = calloc(N+1, sizeof(double));
    rz = calloc(N+1, sizeof(double));
    
    rbi = calloc(3*Nparticle, sizeof(double)); //april_8
    outp1 = malloc(sizeof(char)*300);
    outp2 = malloc(sizeof(char)*300);
    outp2a = malloc(sizeof(char)*300); 
    outp2b = malloc(sizeof(char)*300);  //may_12
    sprintf(outp1, "prop/MSD%d_%d_%.3lf_%.5lf_%d_%d_%d.txt", Nb, Nc, c_norm, flowrate, nt_avg, trace, iterstart);
    sprintf(outp2, "prop/disp%d_%d_%.3lf_%.5lf_%d_%d_%d.txt", Nb, Nc, c_norm, flowrate, nt_avg, trace, iterstart);
    sprintf(outp2a, "prop/disp_a%d_%d_%.3lf_%.5lf_%d_%d_%d.txt", Nb, Nc, c_norm, flowrate, nt_avg, trace, iterstart);
    sprintf(outp2b, "prop/disp_b%d_%d_%.3lf_%.5lf_%d_%d_%d.txt", Nb, Nc, c_norm, flowrate, nt_avg, trace, iterstart);
    dxt = calloc(printprops, sizeof(double));
  	dyt = calloc(printprops, sizeof(double));
  	dzt = calloc(printprops, sizeof(double));//april_8
    dxta = calloc(printprops, sizeof(double));
  	dyta = calloc(printprops, sizeof(double));
  	dzta = calloc(printprops, sizeof(double));//april_8
    dxtb = calloc(printprops, sizeof(double));
  	dytb = calloc(printprops, sizeof(double));
  	dztb = calloc(printprops, sizeof(double));//april_8
    ...
    fx = calloc(N+1, sizeof(double));//april_8
    fy = calloc(N+1, sizeof(double));
    fz = calloc(N+1, sizeof(double));//april_8
    f = calloc(3*(N+1), sizeof(double));
    R = calloc(3*(N+1), sizeof(double));//april_8
    D = calloc(nt_avg,sizeof(double));
    for(i=0;i<nt_avg;i++){
        D[i] = calloc(9*num_bins,sizeof(double));
    }
    M = calloc(9*(N+1)*(N+1),sizeof(double));//april_8
    ...
    // exit(1);
    Ct = calloc(3*(N+1),sizeof(double));//april_8
    ...
    tauxx = 0.0;
    tauyy = 0.0;
    rowsum = calloc(3*(N+1),sizeof(double));//april_8
    count = calloc(num_bins,sizeof(double));//june22
    start_ij = calloc((N+1)*(N+1),sizeof(double));//april_8 june_22
    bp = calloc(sbins+1,sizeof(double));
    brun = calloc(sbins+1,sizeof(double));
    gwcount = calloc(sbins+1,sizeof(double));//june_22
    p = sqrt(2.0/dt);
    pc = sqrt(2.0*dt);
    ...
    if(restart==1 &&iterstart==0){
        ...
        if(DCfile){
            ...
            for(i=0;i<sbins+1;++i){
                ...
                brun[i] *= gwcount[i];
                for(j=0;j<3*(N+1);j++){//april_8
                    Crun[i][j] *= ((Nc+1)*gwcount[i]);//may_1
                }
            }
            fclose(DCfile);
        }
